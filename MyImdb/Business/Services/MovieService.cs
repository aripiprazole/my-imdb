...
namespace MyImdb.Business.Services {
	public class MovieService {
		...
		public async Task<Movie> SelectById(Guid id) {
			return await movieRepository.SelectById(id) ?? throw ApiException.Builder()
				.Build(ErrorCodes.MovieNotFound, new { id });
		}
		...
		public async Task<Movie> Create(int rank, string title, int year, string storyLine, Guid genreId) {
			var genre = await genreRepository.SelectById(genreId) ?? throw ApiException.Builder()
				.Build(ErrorCodes.GenreNotFound, new { id = genreId });
